...
class TraineeDetail extends Component {
  ...
  connectApi = async () => {
    ...
    try {
      const res = await callApi('/trainee', 'GET', {}, storedToken);
      this.setState({
        response: res.data.data.records,
      });
    } catch (error) {
      console.log(error);
    }
  ...
  render() {
    ...
    if (response.some(trainee => trainee._id === match.params.id)) {
      const item = response.filter(trainee => trainee._id === match.params.id);
      return (
        <>
